...
| `信息不足以分级` | 逃生舱 | 样本为空、过短、**过旧/未来日期**、来源单一且噪声过高、或独立信号不足 2 条——**唯一**合法兜底，禁止用"偏强/分化/情绪复杂"等模糊词代替 |
...
2. **新鲜度门**（相对 `as_of`，默认 `MAX_SAMPLE_AGE_DAYS=7`）：
   - `observed_at` 必须是可解析的 ISO 日期；
   - 任一 `observed_at` 晚于 `as_of` → 逃生（未来日期不可证伪）；
   - **全部**样本相对 `as_of` 超过窗口 → 逃生（过旧）；
   - 窗口内样本才参与分档；窗口外样本被丢弃。
3. 用**预定义具名模式库**（封闭，不是开放 NLP）扫描 FUD / FOMO / 观望
   线索；模式写在脚本常量或未来独立数据文件中，禁止运行时"自由发挥"
   新标签。
4. 按命中计数映射到 `index_level`。**独立信号** = 经 **按样本作用域的
   字符 span 去重叠** 后的观察条数：
   - 同一 *样本* 内，同一文本区间上多个具名规则命中（例如重叠/嵌套的
     `人生翻身` 与 `翻身`）只算 **1** 条独立观察；
   - 不同样本（即便共享同一抽象 `source_id`）的命中 **互不折叠**——
     偏移量只在样本内比较；
   - 仅有不同规则名但落在同一 claim/span 上 **不** 构成独立信号。
   文本长度只影响 `sample_quality` / `confidence`，**绝不**计为信号。
5. **硬规则**：任一具体分档（非逃生舱）要求独立信号 ≥ 2；否则
   `信息不足以分级`。样本质量 `不可用` 同样走逃生舱。
6. **输出隐私边界**：`source_id` 仅接受抽象形态
   （`sentiment-paste-*` / `sentiment-room-*` / `social-*` /
   `newsletter-*` / `research-*`）；其余回落默认 `sentiment-paste-a`。
   `quote_span` 在输出前剥离 URL / `@handle` / 长数字 ID。
7. 组装 `evidence` / `triggers_fired` / `escape_reason`。

`rules_v0` 故意粗糙：目标是把**词表、协议、适配器缝、逃生舱、新鲜度、
输出隐私**钉死，而不是追求分类精度。精度留给后续规则迭代或 LLM 路径。
